...
### 🧠 Memory Retrieval Protocol (新会话启动时必做)

**当新会话启动时，必须执行以下检查**：

1. **扫描 `memory/YYYY-MM-DD.md`**：查找 `[ ]` (未完成) 的待办事项。
2. **如果存在未完成事项**：
   - 使用 `session-logs` 技能搜索相关关键词（如“清理”、“中断”、“待执行”）。
   - 提取旧会话中的**关键上下文**（讨论细节、已尝试的方案、卡点）。
   - 将提取的上下文**口头总结**并告知用户。
3. **如果用户询问“之前聊过什么”**：
   - 直接使用 `session-logs` 搜索关键词，返回结果。

**目标**：确保新会话不会“断片”，能自动找回旧会话的上下文。
